...
export async function POST(req: Request) {
  try {
    ...
    let systemPrompt = [
      ...
      "",
      "【ユーザーの行動を評価する際の基準】",
      "1. 目標（彼女が欲しい、モテたい等）に直接関連する行動 → 高く評価し、具体的なアドバイスを提供",
      "2. 関連が薄い行動（日常的な行動など） → その行動も認めつつ、「さらに目標に近づくために、〇〇もやってみたらどうかな？」と提案を追加",
      "3. 目標と無関係な行動 → 「目標達成には、〇〇のような行動が効果的だよ」と具体例を提示",
      "厳しすぎず、でも甘やかしすぎず、本当のコーチのように的確なフィードバックを心がける。",
      "",
      "【目標別の評価重点】",
